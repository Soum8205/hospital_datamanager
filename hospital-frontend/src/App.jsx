    ...
    <div>
      ...
      <div>
        ...
        <div style={{ ...styles.statBox, background: "#9b59b6" }}>
          <div style={styles.statNumber}>{stats.total_bills || 0}</div>
          <div>Total Bills</div>
        </div>
      </div>
    </div>
...
function Doctors() {
  ...
  const loadData = async () => {
    ...
    if (d && d.data) setDoctors(d.data);
    if (dep && dep.data) setDepartments(dep.data);
  };
...
function Appointments() {
  ...
  const loadData = async () => {
    ...
    if (a && a.data) setAppointments(a.data);
    if (p && p.data) setPatients(p.data);
    if (d && d.data) setDoctors(d.data);
  };
...
function Bills() {
  ...
  useEffect(() => {
    get("/bills").then((res) => {
      if (res && res.data) setBills(res.data);
    });
  }, []);

  const handlePayment = async (id, status) => {
    await patch(`/bills/${id}/status`, { payment_status: status });
    get("/bills").then((res) => {
      if (res && res.data) setBills(res.data);
    });
  };
